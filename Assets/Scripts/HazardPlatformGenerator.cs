...
public class HazardPlatformGenerator : MonoBehaviour
{
    ...
    public GameObject hazardsCloud; //Matt: Storm cloud prefab
    ...
    void SpawnClouds(float topEdgeY)
    {
        ...
        Vector3 rightEdge = mCamera.ViewportToWorldPoint(new Vector3(1f, 0.5f, 10f));
        float xSpot;
        GameObject cloudToSpawn; //Holds cloud instance so it can be switched between platform and hazard

        for (int i = 0; i < spawnPoints.Length; i++)
        {
            int selection = Random.Range(0, templatesCloud.Length);
            float randomSpawn = Random.Range(0f, 1.0f);

            if (randomSpawn <= 0.9f)
            {
                cloudToSpawn = templatesCloud[selection];
            }
            else
            {
                cloudToSpawn = hazardsCloud;
            }

            Vector3 size = cloudToSpawn.GetComponent<SpriteRenderer>().sprite.bounds.size * templates[selection].transform.localScale.x;
            xSpot = Random.Range(leftEdge.x + (size.x / 2f), rightEdge.x - (size.x / 2f));
            ...
            if (!DoesOverlap(spawnVector, size))
            {
                if (platformInstances[i] == null || spawnVector.y - platformInstances[i].transform.position.y > spawnDist) //Matt
                {
                    platformInstances[i] = Instantiate(cloudToSpawn, spawnVector, Quaternion.identity) as GameObject;
